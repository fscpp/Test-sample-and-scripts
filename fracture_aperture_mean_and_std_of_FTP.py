...
for i in data:
    ...
    if len(indx)>7000: #Analyze only the 5 big fractures
        ...
        ma = np.ravel(ap[:]); ap_ma_all = ma[ma>0.].copy(); ap_ma_mean = np.mean(ap_ma_all); s_ma = round(np.std(ap_ma_all),2)
        ornt = np.array(ornt, dtype=int); da = ornt[:,0].copy(); dd = ornt[:,1].copy()
        print ("{} measurements made ({} H and {} V):\nMean MA:\t{} [{} data points]\tsd={}\n"
               .format(len(ap), n_h, n_v, round(ap_ma_mean,2), len(ap_ma_all), s_ma))
        ap_all.append(ap)